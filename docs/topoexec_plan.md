...
Post-alpha status: persistent `thread_pool` worker-pool v1, runtime priority/admission ordering v1, cooperative cancellation/timeout observation v1, opt-in `fixed_rate` wall-clock cadence v1, and async edge `policy.max_inflight` admission now exist with tests. Remaining work is OS-level scheduling policy, advanced starvation aging, hard timeout preemption, and TSAN CI.
