...
| `GraphSpec`, `LaneSpec`, `EdgeSpec`, `TriggerPolicySpec`, `CompositeLoopSpec`, `GraphHierarchyEntry` | stable-v0.2 for current fields | Additive fields are allowed only when schema/runtime meaning stays compatible. `GraphHierarchyEntry` records compile-time `subgraphs[]` expansion metadata only; it does not imply runtime nesting. Trigger v2 `watermark`, `condition`, `debounce`, and `rate_limit` policy fields are additive preview fields and may be refined before beta. |
