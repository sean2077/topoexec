...
| Channel capacity and overflow | `v0.2 stable` | Channels and async admission are bounded. Overflow/drop/reject/deadline/stale outcomes are observable through metrics and health counters, not hidden blocking or recursive control flow. |
...
| Scheduler event-loop lane | `v0.1 stable` | `event_loop` is the deterministic default lane. It executes compiled ready work without OS scheduler claims. |
| Scheduler fixed-rate lane | `experimental` | Current behavior is deterministic/simulated over bounded runtime ticks by default, with opt-in cooperative wall-clock sleeping via `wall_clock_enabled`, `period_ms`/`hz`, and `overrun_policy`. Tick, overrun, skipped, jitter, blocked-duration, and max-lateness metrics are observable. Independent lane threads, OS jitter control, and hard real-time guarantees are future work. |
| Scheduler thread-pool lane | `experimental` | Current behavior is run-scoped persistent worker-pool execution for ready invocations with bounded FIFO queue admission, non-reentrant serialization, worker-id trace attributes, and queue/worker metrics. Priority queue ordering, OS scheduling policy, and timeout preemption are future work. |
...
| State/config snapshot stores | `experimental` | Stores are epoch-boundary committed and observable, but reset/restore/hot-reload transaction APIs are future work. |
| TaskExecutor deterministic helper | `experimental` | Current task execution is deterministic and bounded. Threaded executor preview is future work. |
| Cooperative cancellation and timeout | `future extension` | Stop tokens exist for scheduler/runtime stopping. Component/task/loop cooperative cancellation and honest timeout propagation are future work; no hard preemption is implemented. |
