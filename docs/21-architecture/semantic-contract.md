...
| Runtime observer API | `v0.2 stable` | `RuntimeRunnerOptions::observers` delivers best-effort result/metric/trace/health/error records after run assembly; observer failures are non-fatal diagnostics. Exporter adapters should consume this surface; G58/G59 prove the shape with dependency-free telemetry previews. |
