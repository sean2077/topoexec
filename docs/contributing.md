...
| Public API change | Installed headers, CMake targets, CLI JSON, metrics, trace, diagnostics, schema, or package metadata changes. | `docs/api-change-checklist.md`, `docs/public-api.md`, versioning note, downstream smoke when embedder-facing. |
...
| Schema field | Adds or changes graph YAML/JSON shape. | `docs/schema-v2-notes.md` classification, schema docs/JSON, schema contract tests, migration note if breaking. |
...
1. The current contract section (`docs/runtime-semantics.md`,
   `docs/semantic-contract.md`, `docs/schema-v1.md`, or a feature page).
...
4. Add a CTest smoke and update `docs/examples.md` / `examples/README.md`.
...
4. Update `docs/metrics.md`, tests, and goldens when CLI output changes.
...
1. Classify the field in `docs/schema-v2-notes.md` before editing the schema.
...
3. Update `schema/topoexec.schema.v1.json`, `docs/schema-v1.md`, examples, and
...
Use `docs/api-change-checklist.md`. Public API includes installed headers,
...
renames, or behavior changes in `CHANGELOG.md`, `docs/versioning.md`, and the
...
- Human maintainers choose public tags using `docs/release-runbook.md` and
...
- Schema/semantic changes require `docs/schema-v2-notes.md` and
  `docs/semantic-contract.md` review before implementation.
