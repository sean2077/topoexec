...
- Commit: `e873c62 Make TopoExec schema semantics executable`
- Branch state: `main` is synced with `origin/main` (`git rev-list --left-right --count HEAD...origin/main` returned `0 0`)
- Working tree: clean (`git status --short --branch` returned `## main...origin/main`)
- Verification gate: `ctest --test-dir build --output-on-failure` returned 22/22 passing tests
...
| 5 | Delay/state/async edges do not create immediate SCCs | `src/graph.cpp`, `docs/runtime-semantics.md` | `Graph.DelayEdgeBreaksImmediateCycle`, `Graph.NonImmediateFeedbackEdgesDoNotCreateImmediateSccs` |
...
| 15 | Copy policies reject large payload copy and avoid copies for shared/loaned views | `src/channel.cpp`, `src/payload.cpp` | `Channel.CopyPolicyRejectsLargePayloads`, `Channel.SharedAndLoanedViewDoNotCopyPayloads` |
...
| 20 | Async worker app proves task-ready delivery and drop policy | `examples/apps/async_worker/main.cpp` | CTest asserts `async_drop_count=1`; direct app output prints `task_ready_epoch=2` and `async_drop_count=1` |
...
## Deferred Scope Not Claimed Complete

The following items remain intentionally deferred because `docs/spec.md` frames them as later or optional, and current docs do not claim otherwise:

- ROS 2, OpenTelemetry, Prometheus, Perfetto, Python, and other adapters
- Real threaded `worker_pool` scheduling and rich async `max_inflight` worker policy
- Timestamp slop enforcement for `time_sync`
- A real loaned-buffer API behind `loaned_view`
- Richer multi-writer state-store conflict policy beyond current validation/lint visibility
