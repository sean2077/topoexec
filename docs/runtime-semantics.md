...
An `Invocation` carries event kind, trigger kind, channel id, local correlation id, ready input names, payloads by port, batch payloads when relevant, timing fields, sequence, budget, lane, priority, deadline metadata, and stop-token access.
