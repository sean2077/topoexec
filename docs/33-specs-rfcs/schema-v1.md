...
Runtime visibility rules for edge kinds, epochs, transactions, commits, triggers, and CompositeLoop ownership are defined in [runtime-semantics.md](../21-architecture/runtime-semantics.md). Versioned runtime meaning is tracked separately in [semantic-contract.md](../21-architecture/semantic-contract.md); schema v1 controls graph shape, while the semantic contract controls behavior meaning.
...
[Graph templates](../11-user-guide/graph-templates.md) for the full contract.
...
nesting depth are documented in [Defensive input handling](../24-testing/defensive-input.md).
...
For editor integration and YAML-language-server setup, see [Editor and Schema UX](../41-development-tools/editor-schema.md).
