...
```text
47/47 CTest tests passed in the default local RelWithDebInfo GCC run, including normalized CLI golden outputs, schema contract smoke, docs command smoke, state/config snapshot tests, example validation/run smokes, benchmark JSON smokes, schema tooling smokes, and doctor JSON smoke.
```
...
```text
47/47 CTest tests passed; downstream topoexec::runtime package smoke executable linked only topoexec::runtime and exited 0.
```
